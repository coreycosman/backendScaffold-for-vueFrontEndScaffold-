...
UserSchema.pre("save", function(next) {
  // only hash the password and confirmation if it has been modified (or is new)
  if (!this.isModified("password") && !this.isModified("confirmation"))
    return next();

  this.password = bcrypt.hashSync(this.password, bcrypt.genSaltSync(10), null);
  this.confirmation = bcrypt.hashSync(
    this.password,
...
UserSchema.methods.generateSafePassword = function(body) {
  return new Promise((resolve, reject) => {
    let hashed = bcrypt.hashSync(body.password, bcrypt.genSaltSync(10), null);
    body.password = hashed;
    body.confirmation = hashed;
    resolve(body);
  });
};
